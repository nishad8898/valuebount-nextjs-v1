import IntegrationList from '@/data/integrationData'
import Image from 'next/image'
import FadeUpOneByOneAnimation from '../animations/FadeUpOneByOneAnimation'

const TopIntegration = () => {
  const { IntegrationData } = IntegrationList
  return (
    <section className="mt-20">
      <div className="mb-[100px] text-center">
        <p className="mb-4 font-medium uppercase">A COMBINATION OF INNOVATION AND EXPERTISE</p>
        <h2 className="mb-10 max-lg:mb-10">
          Make productivity easier with <br /> <span className="text-[#FF4500]">Value</span>bound
        </h2>
        <p className="mx-auto mb-12 max-w-[590px] max-lg:mb-10">
          Empowering clients with Drupal expertise that exceeds expectations and spans across continents.
        </p>
      </div>

      <section className="relative pb-150">
        <div className="absolute -top-[300px] left-1/2 -z-10 h-[550px] w-full -translate-x-1/2  bg-[url('/images/hero-gradient.png')] bg-cover bg-center bg-no-repeat opacity-70 md:hidden"></div>
        <div className="container ">
          <div className="relative z-10 mx-auto">
            <div className="absolute left-1/2 top-1/2 -z-10 flex -translate-x-1/2 -translate-y-1/2 max-md:hidden max-md:flex-col">
              <div className="rounded-full bg-primary-radial  blur-[145px] max-1xl:h-[335px]  max-1xl:w-[335px] 1xl:h-[442px] 1xl:w-[442px]"></div>
              <div className="-ml-[170px] rounded-full  bg-primary-radial blur-[145px]  max-1xl:h-[335px] max-1xl:w-[335px] max-md:ml-0 1xl:h-[442px] 1xl:w-[442px]"></div>
              <div className="-ml-[170px] rounded-full  bg-primary-radial blur-[145px]  max-1xl:h-[335px] max-1xl:w-[335px] max-md:ml-0 1xl:h-[442px] 1xl:w-[442px]"></div>
            </div>
            <div className=" grid grid-cols-4  gap-8 max-lg:grid-cols-2 max-md:grid-cols-1">
              {IntegrationData.map((items, i) => (
                <FadeUpOneByOneAnimation
                  i={i}
                  className="rounded-medium bg-white p-2.5 shadow-box dark:bg-dark-200"
                  key={items.id}>
                  <div className="rounded border border-dashed border-gray-100 p-10 text-center dark:border-borderColor-dark ">
                    <Image
                      src={items.image}
                      alt="value image"
                      className="inline-block h-auto w-auto"
                      width={120}
                      height={120}
                    />
                    {/* <h3 className="mb-2.5">{items.title}</h3>
                    <p>{items.details}</p> */}
                  </div>
                </FadeUpOneByOneAnimation>
              ))}
            </div>
          </div>
        </div>
      </section>
    </section>
  )
}

export default TopIntegration
